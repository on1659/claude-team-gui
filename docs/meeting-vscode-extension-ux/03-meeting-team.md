...
### 1. 개발 전 (As-Is) — 현재 CLI 기반 워크플로우

개발자가 AI 팀 회의를 실행하려면 다음 과정을 거친다:

1. VS Code에서 코딩하다가 팀 회의가 필요해짐
2. 터미널을 열거나 별도 터미널 앱으로 전환
3. Claude Code CLI에서 `/meeting-team` 명령어 입력
4. 주제를 텍스트로 타이핑
5. 8명의 에이전트가 **순차적으로** 실행되며 결과가 터미널에 흘러감
6. 스크롤해서 각 에이전트 결과를 확인
7. 결과를 복사해서 별도 문서에 붙여넣기
8. 다시 VS Code로 전환해서 코딩 재개

**소요 시간**: ~3~5분 (회의 실행) + ~2분 (결과 정리) = 총 5~7분
**컨텍스트 전환**: 최소 2회 (VS Code → 터미널 → VS Code)
**결과 형식**: 플레인 텍스트 (구조화 없음)

### 2. 개발 후 (To-Be) — VS Code Extension 워크플로우

1. VS Code에서 코딩하다가 팀 회의가 필요해짐
2. **사이드바**에서 팀원 카드를 확인하고, 참여할 멤버를 클릭으로 선택/해제
3. 주제 입력란에 주제 작성 (또는 코드 선택 → 우클릭 → 주제 자동 채움)
4. 회의 모드 선택 후 **[회의 시작]** 클릭
5. 에디터 영역에 **Panel**이 열리며 8개 카드가 동시에 나타남
6. 각 카드에 **실시간 스트리밍**으로 결과가 채워지는 것을 시각적으로 확인
7. 완료된 카드는 녹색으로 변하고, 에러 카드는 [재시도] 버튼 표시
8. 전체 완료 후 결과를 **마크다운으로 복사** 또는 **파일로 저장**
9. Panel 탭을 닫고 바로 코딩 재개 (같은 에디터 안)

**소요 시간**: ~1~2분 (8명 병렬 실행) + ~0분 (결과 이미 구조화) = 총 1~2분
**컨텍스트 전환**: 0회 (모든 작업이 VS Code 안에서 완료)
**결과 형식**: 카드별 구조화 + 마크다운 복사 지원
...
```text
Step 1: 개발자가 새 API 엔드포인트를 설계해야 함
Step 2: 사이드바에서 태준(BE), 미래(FE), 윤서(QA) 3명만 선택
Step 3: 주제: "사용자 알림 API 설계 — REST vs WebSocket"
Step 4: [회의 시작] 클릭
Step 5: Panel에 3개 카드가 열리며 각자 관점에서 의견 스트리밍
Step 6: 태준은 API 구조, 미래는 FE 연동, 윤서는 테스트 관점 제시
Step 7: 완료 후 [마크다운 복사] → 설계 문서에 붙여넣기
Step 8: 바로 코딩 시작
```
...
```text
Step 1: 개발자가 복잡한 함수를 작성함
Step 2: 해당 코드 블록을 드래그 선택
Step 3: 우클릭 → "이 코드로 팀 회의" 선택
Step 4: 주제가 자동 채워짐: "[processStream]: 리뷰" + "코드 42줄 첨부됨"
Step 5: 전체 8명 선택된 상태로 [회의 시작]
Step 6: 8명이 각자 관점에서 코드 리뷰 의견 제시
Step 7: QA는 엣지케이스, UX는 에러 메시지, BE는 성능을 지적
Step 8: 결과를 보고 코드 수정
```
...
```text
Step 1: 사이드바 하단의 비용 예측기가 "8명 × meeting-team ≈ $0.35" 표시
Step 2: 예산이 부족하면 팀원 3명으로 줄임 → "$0.12"로 갱신
Step 3: 납득되면 [회의 시작]
Step 4: 실행 중에도 사이드바에서 현재까지 사용된 토큰 확인 가능
```

### 4. 기대 효과

| 항목 | CLI (Before) | Extension (After) | 개선 |
| ---- | ------------ | ----------------- | ---- |
| 회의 실행 시간 | 3~5분 (순차) | 1~2분 (병렬) | **60% 단축** |
| 컨텍스트 전환 | 2회 이상 | 0회 | **제거** |
| 결과 정리 시간 | 2분 (수동 복사) | 0분 (구조화 제공) | **제거** |
| 참여자 선택 | 불가 (전원 고정) | 클릭으로 선택/해제 | **유연성 확보** |
| 비용 파악 | 실행 후에야 알 수 있음 | 실행 전 예측 표시 | **예측 가능** |
| 에러 대응 | 전체 재실행 | 실패 카드만 재시도 | **비용 절감** |
| 실행 상태 파악 | 터미널 스크롤 | 카드별 상태 아이콘 | **직관적** |

### 5. 주의사항/한계 (v1)

- **회의 모드는 meeting-team 1종만** 지원. meeting-multi, meeting-agent는 v2
- **프로필 편집 UI 없음** — 팀원 추가/수정은 `profiles.json` 직접 편집 필요
- **비용 예측은 정적 계산** — 실행 중 실시간 반영은 v2
- **결과 저장은 마크다운 복사만** — 자동 파일 저장, 히스토리 관리는 v2
- **Anthropic API 키 필수** — 키 없으면 모든 기능 비활성화
- **rate limit 주의** — Free tier에서 8명 동시 실행 시 429 에러 가능. 유료 플랜 권장
- **오프라인 사용 불가** — API 호출 기반이므로 인터넷 연결 필수
