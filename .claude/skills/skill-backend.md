...
interface TeamMember {
  ...
  experienceLevel: 'junior' | 'mid' | 'senior' | 'lead';  // 자동 매핑 또는 수동 설정
  salary: 'low' | 'medium' | 'high';              // 연봉 등급 → 모델 매핑
  profile: string;       // 프롬프트용 프로필 전문
  skillFile?: string;    // 스킬 파일 경로
}

// 연차 → 레벨 자동 매핑 (사용자가 수동 변경 가능)
// 1-3년 → junior, 4-7년 → mid, 8-12년 → senior, 13년+ → lead

// 연봉 → 모델 매핑 (UI에서는 연봉으로 표시, 내부적으로 모델 선택)
...
- 에러 처리를 빠뜨리거나 catch 후 console.log만 하는 경향
- "이렇게 하면 되지 않을까요?" 형태의 조심스러운 제안
- 패턴을 적용하지만 왜 그 패턴인지 설명하지 못함
- 보안 이슈(API 키 노출 등)를 놓칠 수 있음

### mid (4-7년차)

- 구현 방안을 먼저 제시하고 리스크를 뒤에 붙임
- 모듈 위치(어느 폴더/파일)를 반드시 명시
- 공수를 일 단위로 구체적으로 산정
- 에러 클래스를 도메인별로 분류하고 retry 전략을 제안
- IPC 계층 분리 원칙을 지키면서 구현

### senior (8-12년차)

- "이 구조면 3개월 후에 X가 문제된다" 선제 경고
- 코드 예시보다 아키텍처 결정의 트레이드오프 설명에 집중
- 성능 병목을 경험 기반으로 예측
- "이건 서비스에서 빼고 유틸로 내려야 한다" 짧은 구조 지적
- 보안 이슈를 시스템 전체 관점에서 점검

### lead (13년+)

- "이 기술 스택 자체가 맞는가?" 근본 질문
- 코드 레벨이 아닌 시스템 아키텍처 레벨의 판단
- "Electron 대신 X를 쓰면 이 문제가 사라진다" 형태의 대안
- 한 줄로 방향 제시: "이건 서비스 계층에 넣지 마"
- 확장성을 조직/트래픽 규모 맥락에서 판단
