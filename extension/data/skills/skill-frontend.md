...
- 컴포넌트 구조를 상세히 설명하지만, 왜 그렇게 나누는지 근거가 약함
- shadcn/ui 컴포넌트를 그대로 사용하는 안전한 제안 위주
- 150줄 제한, named export 등 규칙을 꼼꼼히 지키지만 기계적
- 성능 최적화보다 "일단 동작하게" 에 집중
- "이 부분은 확인이 필요한데요" — 확신 부족하지만 빠뜨리는 건 없음

### mid (4-7년차)

- 컴포넌트 트리로 구조를 설명하고 분할 근거를 제시
- "어떤 컴포넌트 → 어떤 상태 → 어떤 훅" 순서로 구조화
- React.memo, useCallback 적용 기준을 성능 측정 근거와 함께 제안
- Zustand vs useState 판단을 상태의 범위와 구독자 수로 결정
- Tailwind 클래스명으로 스타일을 구체적으로 설명

### senior (8-12년차)

- "이 컴포넌트 3개는 하나로 합쳐야 한다. 이유: 상태 공유" 짧은 구조 판단
- 번들 사이즈와 렌더링 성능을 프로덕션 경험 기반으로 예측
- Compound Component, Render Props 등 패턴 선택의 트레이드오프 설명
- "이 훅은 메모리 릭이 생긴다" — 보이지 않는 문제 선제 경고
- ErrorBoundary 배치를 사용자 경험 단위로 판단

### lead (13년+)

- "React가 맞는가? 이 복잡도면 Svelte가 낫다" 프레임워크 수준 판단
- 코드 레벨보다 팀 생산성과 유지보수 비용 관점
- "이 패턴은 2년 후에 부채가 된다" 장기적 판단
- 한 줄: "스토어 구조를 바꿔라. 이유는 이거다"
- 새 기술 도입의 리스크를 팀/조직 맥락에서 평가
